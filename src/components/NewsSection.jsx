...
import { Calendar, User, ArrowRight, X, Newspaper, Clock, ExternalLink } from 'lucide-react';
...
export default function NewsSection({ newsList = [] }) {
  ...
  const handleCardClick = (news) => {
    // Jika berita memiliki link dan tidak memiliki konten panjang, langsung buka link
    if (news.link && (!news.content || news.content.trim().length === 0)) {
  ...
  return (
    <section id="berita" ref={sectionRef} className="py-20 bg-slate-50 border-t border-slate-100 relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* News Grid (Otomatis responsive & rapi untuk 1 maupun banyak berita) */}
        {newsList.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-3xl border border-slate-200 max-w-md mx-auto shadow-sm">
            <Newspaper className="w-12 h-12 text-slate-300 mx-auto mb-3" />
            <p className="text-slate-500 text-sm font-semibold">Belum ada berita yang diterbitkan.</p>
          </div>
        ) : (
            ...
            <div className="p-6 sm:p-10 space-y-6">
              <div>
                <div className="flex items-center gap-3 text-xs text-slate-500 font-semibold mb-2">
                  <span>Ditulis oleh: <strong className="text-brand-navy">{activeNewsModal.author || 'Tim KKN'}</strong></span>
                  <span>•</span>
              ...
              {/* Tautan Eksternal jika ada */}
              {activeNewsModal.link && (
                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3">
                  <div className="text-xs text-slate-600">
                    <span className="font-bold text-brand-navy block">Sumber / Tautan Berita Lengkap:</span>
                    <span className="text-slate-500 break-all">{activeNewsModal.link}</span>
                  </div>
                  <a
                    href={activeNewsModal.link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-shrink-0 px-4 py-2 rounded-xl bg-gradient-to-r from-brand-gold to-amber-600 hover:from-amber-600 hover:to-brand-gold text-white font-bold text-xs flex items-center gap-2 shadow-sm transition-all"
                  >
